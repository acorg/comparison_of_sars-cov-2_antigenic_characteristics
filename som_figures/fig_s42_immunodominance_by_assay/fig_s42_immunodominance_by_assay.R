...
line_data %>%
  mutate(
    map = factor(map, levels = levels(curated_data$map))
  ) %>%
  filter(
    ) -> line_data_mf
...
geom_pointrange(
  ...
  facet_grid(map ~ sr_group, scales = 'free_y', space = 'free_y') -> gp
...
textdata %>%
  ...
  facet_grid(map ~ variable, scales = 'free_y', space = 'free_y', switch = 'y',
             labeller = labeller(map = pretty_labels)) -> l

ggarrange(l, NULL, gp,
          ncol = 3, nrow = 1, widths = c(0.5, -0.03, 1), align = 'h')

ggsave('som_figures/fig_s42_immunodominance_by_assay/fig_s42_immunodominance_by_assay_panelA.png', width = 13.1, height = 7)
...
ggsave('som_figures/fig_s42_immunodominance_by_assay/fig_s42_immunodominance_by_assay_panelB.png', width = 13.1, height = 7)
