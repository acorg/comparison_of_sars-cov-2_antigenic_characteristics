...
png('som_figures/fig_s63_individual_maps_merged_map_procrustes/fig_s63_individual_maps_merged_map_procrustes.png', 30, 15, units = 'in', res=300, pointsize = 12)
...
for (mapName in list('duke', 'fda', 'innsbruck', 'amc', 'emory', 'geneva',
  ...
  plot(p, xlim = c(-1, 9), ylim = c(-3, 4), fill.alpha = 1, plot_labels = FALSE,
       outline.alpha = 1, grid.col = '#cfcfcf', plot_stress = FALSE,
       grid.margin.col = 'black', show_error_lines = FALSE, cex = 1.5)
  
  text(9, 3.5, pretty_labels[mapName(m_)], cex = 2, pos = 2, font = 2)
  text(9, -2.6, dataset_labels_sub[mapName(m_), ], cex = 1.8, pos = 2, font = 1)
}
