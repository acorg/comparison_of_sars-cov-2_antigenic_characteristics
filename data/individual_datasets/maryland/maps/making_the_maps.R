...
titers <- read.titerTable('data/individual_datasets/maryland/rawdata/table-220203.csv')
sr_groups_info <- read.csv('data/individual_datasets/maryland/rawdata/sr_groups.csv', stringsAsFactors = FALSE)
...
srGroups(map_full) <- factor(
  ...
  levels = c(
    ...
    'B.1.351 vac protein',
    'P.1 vac protein',
    'B.1.617.2 vac protein',
    'B.1.617.2+K417N vac protein'
  )
)

# Edit ag names
agNames(map_full)[agNames(map_full) == 'WA1'] <- '614D'
agNames(map_full)[agNames(map_full) == 'AY.1'] <- 'B.1.617.2+K417N'
agNames(map_full)[agNames(map_full) == 'Omicron'] <- 'BA.1'
...
map_subset <- translateMap(map_subset, c(-0.35, 0))

# Name the map
mapName(map_subset) <- 'maryland'

# Save the map
save.acmap(map_subset, 'data/individual_datasets/maryland/maps/map.ace')
